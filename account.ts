...
import { baseDecode, Logger, parseResultError, DEFAULT_FUNCTION_CALL_GAS, printTxOutcomeLogsAndFailures } from '@near-js/utils';

import { Connection } from './connection';
import { viewFunction, viewState } from './utils';
import { ChangeFunctionCallOptions, ViewFunctionCallOptions } from './interface';
...
// Custom hook to interact with NEAR Account
export function useNearAccount(connection: Connection, accountId: string) {
    const [account, setAccount] = useState<Account>(null);
    const [accountState, setAccountState] = useState<AccountView>(null);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        if (connection && accountId) {
            setAccount(new Account(connection, accountId));
        }
    ...
    const signAndSendTransaction = useCallback(async (options: SignAndSendTransactionOptions): Promise<FinalExecutionOutcome> => {
        if (account) {
            try {
                const result = await account.signAndSendTransaction(options);
                return result;
            } catch (err) {
                setError(err.message);
                throw err;
    ...
    const functionCall = useCallback(async (options: ChangeFunctionCallOptions): Promise<FinalExecutionOutcome> => {
        if (account) {
            try {
                const result = await account.functionCall(options);
                return result;
            } catch (err) {
                setError(err.message);
                throw err;
    ...
    const getAccessKeys = useCallback(async (): Promise<AccessKeyInfoView[]> => {
        if (account) {
            try {
...
// NEAR Account class implementation
